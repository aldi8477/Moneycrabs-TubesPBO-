...
public class MoneyMenu implements Screen{
    private Stage uiStage;
    private Game game;

    public MoneyMenu(Game g)
    {
        game = g;
        create();
    }
    public void create(){
        ...
        background.setTexture( new
                Texture(Gdx.files.internal("tiles-menu.jpg")) );
        uiStage.addActor( background );
        BaseActor titleText = new BaseActor();
        titleText.setTexture( new
                Texture(Gdx.files.internal("money-crabs.png")) );
        titleText.setPosition( 110, 70 );
    ...
    public void render(float dt){
        ...
        if (Gdx.input.isKeyPressed(Keys.S))
            game.setScreen( new com.moneycrabs.game.MoneyCrabs(game) );
        // update
        uiStage.act(dt);
        // draw graphics
